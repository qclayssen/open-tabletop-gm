...
def move(enc: Encounter, roller: Roller, token_ref, square, reactions: dict = None) -> dict:
    ...
    for i in range(len(path) - 1):
        for h in by_step.get(i, []):
            ...
            oa, ctx = _oa(enc, h, t, path[i])
            res = R.attack(h, t, oa, ctx, roller, player_rolls(enc, h, roller))
...
def multiattack(enc: Encounter, roller: Roller, attacker_ref, target_ref, routine: int = None) -> dict:
    ...
    for atk in attacks:
        ...
        res = R.attack(a, target, atk, ctx, roller, player_rolls(enc, a, roller))
        results.append(res)
